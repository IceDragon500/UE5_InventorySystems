...
void UInv_SpatialInventory::OnItemHovered(UInv_InventoryItem* Item)
{
	...
	DescriptionTimerDelegate.BindLambda([this, Item, &Manifest, DescriptionWidget]()
	{
		...
		//启动第二个弹出窗口的计时器 针对第二个物品描述，展示该类型的已装备物品
		...
		GetOwningPlayer()->GetWorldTimerManager().SetTimer(EquippedDescriptionTimer, EquippedDescriptionTimerDelegate, EquippedDescriptionTimerDelta, false);
	});
...
void UInv_SpatialInventory::ShowEquippedItemDescription(UInv_InventoryItem* Item)
{
	...
	if (EquippedGridSlot != nullptr) return; //如果装备的网格槽不等于空指针，在这种情况下我们将返回 ,悬停的物品已经被装备 我们已经在显示它的物品描述

	//但如果它通过了这个检查，我们会判定该物品未被装备 因此需要找到已装备的同类型物品
	auto FoundEquippedSlot = EquippedGridSlots.FindByPredicate([HoveredEquipmentType](const UInv_EquippedGridSlot* GridSlot)
	{
		...
		return IsValid(InventoryItem) ? InventoryItem->GetItemManifest().GetFragmentOfType<FInv_EquipmentFragment>()->GetEquipmentTag() == HoveredEquipmentType : false;
	});
